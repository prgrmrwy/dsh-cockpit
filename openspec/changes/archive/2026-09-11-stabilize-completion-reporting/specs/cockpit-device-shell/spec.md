...
### Requirement: 完成未读 chip 的视觉呈现与常规状态 chip 严格一致

系统 SHALL 使完成未读状态 chip 的静止外观（字号、行高、内边距与边框盒）与其所在设备项的等待审批、等待回答和进行中 chip 完全一致；完成 chip 是可点击的清除控件，但其视觉效果 MUST NOT 在尺寸或醒目程度上大于同组常规状态 chip，MUST NOT 因原生 button 样式（UA 字体、边框）改变渲染盒模型。chip 颜色 MUST 走主题令牌，系统 MUST NOT 为完成 chip 写死主题色值。

#### Scenario: 进行中与完成未读并存
- **WHEN** 一台设备同时存在进行中状态与完成未读状态
- **THEN** 两种 chip 以相同字号与盒模型并排显示，完成 chip 不因是可点击清除控件而更大

#### Scenario: 仅存在完成未读
- **WHEN** 一台设备仅有完成未读、无其它特殊状态
- **THEN** 设备项显示的完成 chip 与常规状态 chip 使用同一盒模型，尺寸不超出常规 chip