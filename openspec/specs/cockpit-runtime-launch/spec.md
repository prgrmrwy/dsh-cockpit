# cockpit-runtime-launch

## Purpose

定义 dsh-cockpit 在 Windows、Linux 与 macOS 上统一的启动与生命周期入口：以仓库内单一 Node.js CLI 提供 bootstrap、build、start、restart、stop、status、install、uninstall 与开发模式，并在跨平台的依赖命令发现、运行目录与端口解析、以及基于实例身份与认证的安全生命周期控制上保持一致行为。
